...
export default function AgenciaPage() {
  const [agencyName] = React.useState("Mi Agencia")
  const [agencyLocation] = React.useState("Madrid, España")
  ...
  const handleAdd = async () => {
    ...
    const res = await fetch("/api/agency", {
      ...
      body: JSON.stringify({ action: "add-client", payload: { handle: newHandle.trim() } })
    })
    const data = await res.json()
    if (res.ok) { setClients(data.clients); setNewHandle("") }
    else setAddError(data.error ?? "Error al añadir")
    setAdding(false)
  }
  ...
  const handleBulkImport = async () => {
    ...
    const res = await fetch("/api/agency", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "bulk-add-clients", payload: { handles } })
    })
    const data = await res.json()
  ...
  const handleSync = async (id: string) => {
    ...
    const res = await fetch("/api/agency", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "sync-client", payload: { id } })
    })
    const data = await res.json()
  ...
  const handleDelete = async (id: string) => {
    ...
    const res = await fetch("/api/agency", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "delete-client", payload: { id } })
    })
    const data = await res.json()
    if (res.ok) setClients(data.clients)
    setDeletingId(null)
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between print-hidden">
        <div>
          <div className="section-pill bg-indigo-500/10 text-indigo-400 border-indigo-500/20 mb-3">
            <Building2 className="size-3" /> Portal Agencia B2B
          </div>
          <h1 className="text-2xl sm:text-4xl font-black tracking-tighter leading-tight text-white" style={{ fontFamily: "var(--font-syne)" }}>
            Gestión de <span style={{ background: "linear-gradient(135deg,#6366f1,#a855f7)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Clientes</span>
          </h1>
          <p className="text-muted-foreground font-medium mt-1 text-sm">
            Cuentas independientes del espacio de trabajo principal.
          </p>
        </div>
      ...
      <Card className="glass border-white/[0.07] print-hidden">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-bold flex items-center gap-2">
            <Plus className="size-4 text-indigo-400" /> Añadir Clientes
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-3">
            <Input
              placeholder="@handle de TikTok..."
              value={newHandle}
              onChange={e => { setNewHandle(e.target.value); setAddError(null) }}
              onKeyDown={e => e.key === "Enter" && handleAdd()}
              className="glass border-white/10 bg-transparent font-mono"
              disabled={adding}
            />
            <Button onClick={handleAdd} disabled={adding || !newHandle.trim()} className="bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-xl shrink-0 px-5">
              {adding ? <Loader2 className="size-4 animate-spin" /> : "Añadir"}
            </Button>
            <Button onClick={() => setBulkOpen(v => !v)} variant="outline" className="glass border-white/10 font-bold rounded-xl shrink-0 gap-1.5 hidden sm:flex">
              <Upload className="size-3.5" /> Importar varios
            </Button>
          </div>
          ...
          {bulkOpen && (
            <div className="space-y-3 p-4 rounded-2xl bg-white/[0.02] border border-white/10">
              <p className="text-xs font-bold text-muted-foreground/70">Pega los handles separados por saltos de línea, comas o punto y coma (máx. 20):</p>
              <textarea
                value={bulkText}
                onChange={e => setBulkText(e.target.value)}
                placeholder={"@cuenta1\n@cuenta2\n@cuenta3"}
                className="w-full h-32 bg-black/30 border border-white/10 rounded-xl p-3 text-sm font-mono text-white resize-none outline-none focus:border-indigo-500/50"
              />
              <div className="flex gap-3">
                ...
                <Button variant="ghost" onClick={() => { setBulkOpen(false); setBulkResults([]) }} className="text-muted-foreground/60 rounded-xl">Cancelar</Button>
              </div>
      ...
      {loading ? (
        ...
        <Card className="glass border-white/[0.07] border-dashed">
          <CardContent className="py-16 text-center text-muted-foreground/50">
            ...
            <p className="text-sm font-medium">Sin clientes todavía.<br/>Añade sus @handles arriba.</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4 print-hidden">
            {[
              { l: "Clientes", v: clients.length, i: Users, c: "text-indigo-400", bg: "bg-indigo-500/10" },
              { l: "Audiencia Total", v: fmt(totalFollowers), i: Users, c: "text-emerald-400", bg: "bg-emerald-500/10" },
              { l: "Vistas Est.", v: fmt(totalViews), i: LineChart, c: "text-pink-400", bg: "bg-pink-500/10" },
            ].map((s, i) => (
              <Card key={i} className="glass border-white/[0.07]">
                <CardContent className="p-4 flex items-center gap-3">
                  <div className={`size-9 rounded-xl ${s.bg} ${s.c} flex items-center justify-center`}>
                    <s.i className="size-4" />
                  </div>
                  <div>
                    <div className="text-xl font-black text-white">{s.v}</div>
                    <div className="text-[9px] uppercase font-black tracking-widest text-muted-foreground/50">{s.l}</div>
                  </div>
                </CardContent>
              </Card>
            ))}
                ...
                <CardContent className="p-5">
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
                    <div className="flex items-center gap-4 flex-1">
                      {client.avatar ? (
                        <img src={client.avatar} alt={client.handle} className="size-12 rounded-xl object-cover shrink-0" />
                      ) : (
                        <div className="size-12 rounded-xl bg-indigo-500/20 text-indigo-400 flex items-center justify-center font-black text-lg shrink-0">@</div>
                      )}
                      <div>
                    ...
                    <div className="flex items-center gap-6">
                      {[
                        { l: "Seguidores", v: fmt(client.followers), c: "text-emerald-400" },
                        { l: "Likes", v: fmt(client.likes || 0), c: "text-pink-400" },
                        { l: "Eng.", v: `${client.engagement}%`, c: "text-violet-400" },
                        { l: "Vistas", v: fmt(client.views || 0), c: "text-blue-400" },
                      ].map((s, i) => (
                      ...
                      <div className="flex items-center gap-1.5 ml-2 print-hidden">
                        <Button variant="ghost" size="icon" onClick={() => handleSync(client.id)} disabled={syncingId === client.id}
                        ...
                        <Button variant="ghost" size="icon" onClick={() => handleDelete(client.id)} disabled={deletingId === client.id}
      ...
      <div className="print-area hidden print:block p-8 bg-black min-h-screen">
        <div className="flex justify-between items-end border-b border-white/20 pb-6 mb-8">
          ...
          <div className="text-right text-xs text-muted-foreground/50">
            <p className="font-bold text-indigo-400">Reporte de Cartera</p>
            <p>{new Date().toLocaleDateString("es-ES", { year:"numeric", month:"long", day:"numeric" })}</p>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[["Clientes", clients.length], ["Audiencia", fmt(totalFollowers)], ["Likes", fmt(totalLikes)]].map(([l, v], i) => (
        ...
        {clients.sort((a, b) => b.followers - a.followers).map(c => (
          <div key={c.id} className="flex justify-between items-center p-4 border-b border-white/10">
            ...
            <div className="flex gap-8 text-right">
              <div><p className="text-[9px] text-muted-foreground/50 uppercase">Seguidores</p><p className="font-black text-white">{c.followers.toLocaleString()}</p></div>
